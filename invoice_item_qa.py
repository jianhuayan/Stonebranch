    ...
    with open(infile3, 'r') as in3:
        ...
        for i, row in enumerate(csv_reader3):
            ...
            if i != 0:
                ...
                header = row
    print(customer_map)
    # then look up each invoice code in customer_map
    with open(outfile2, 'r') as out2:
        ...
        with open(outfile3, 'w') as out:
            ...
            for i, row in enumerate(csv_reader4):
                ...
                if i != 0:
                    invoice_code = row[1]
                    invoice_item_list = customer_map[invoice_code]
                    # now each entry in customer map can potentially have multiple invoice items so we loop through those
